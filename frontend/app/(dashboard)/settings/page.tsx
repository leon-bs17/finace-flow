      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-ink">{t("settings.languageCurrency") ?? "Idioma e Moeda"}</CardTitle>
          </div>
        </CardHeader>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-ink">{t("settings.language") ?? "Idioma"}</label>
            <select
              value={locale}
              onChange={(e) => setLocale(e.target.value as Locale)}
              className="h-10 rounded-xl border border-border bg-surface px-3 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-moss-500/50"
            >
              {locales.map((l) => (
